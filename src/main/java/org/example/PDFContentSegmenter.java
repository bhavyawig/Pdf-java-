...
public class PdfSegmentController {

    @Autowired
    private PdfSegmentService pdfSegmentService;

    @PostMapping("/segment")
    public ResponseEntity<?> segmentPdf(@RequestParam("file") MultipartFile file,
                                        @RequestParam("cuts") int cuts) {
        try {
            // Convert MultipartFile to InputStream for PDF processing
            InputStream pdfInputStream = file.getInputStream();
            
            // Call service to handle segmentation
            List<File> segmentedFiles = pdfSegmentService.segmentPdf(pdfInputStream, cuts);
            
            // Return the segmented PDF files
